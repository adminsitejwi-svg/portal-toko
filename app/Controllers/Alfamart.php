...
class Alfamart extends BaseController
{

    public function index()
    {
        $model = new \App\Models\AlfamartModel();

        $data['alfamart']           = $model->orderBy('id', 'DESC')->findAll();
        ...
        $data['Alfamidi']      = (new \App\Models\AlfamidiModel())->findAll();
        $data['Lawson']         = (new \App\Models\LawsonModel())->findAll();


        return view('Alfamart/index', $data);
    }
    public function create()
    {
        ...
        return view('Alfamart/FormAlfa', $data);
    }
    ...
    public function save()
    {
        ...
        $modelCek = new \App\Models\AlfamartModel();
        $kodeToko = trim((string) $this->request->getPost('kode_toko'));
        if ($kodeToko !== '' && $modelCek->where('kode_toko', $kodeToko)->first()) {
            return redirect()->back()->withInput()
                ->with('error', 'Data Toko Alfamart dengan kode toko "' . $kodeToko . '" sudah ada. Data tidak boleh ganda.');
        }
        ...
        $model->save([
            ...
            // Toko
            'nama_alfamart'         => $this->request->getPost('nama_alfamart'),
            'kode_toko'             => $this->request->getPost('kode_toko'),
            'alamat_alfamart'       => $this->request->getPost('alamat_alfamart'),
        ...
        return redirect()->to('/Alfamart')
            ->with('success', 'Data Toko Alfamart berhasil disimpan.');
    }
    ...
    public function delete($id)
    {
        ...
        $model->delete($id);
        return redirect()->to('/Alfamart')->with('success', 'Data berhasil dihapus.');
    }
    ...
    public function edit($id)
    {
        ...
        if (!$data['midi']) {
            return redirect()->to('/Alfamart')->with('error', 'Data tidak ditemukan.');
        }
        ...
        return view('Alfamart/EditFormAlfa', $data);
    }
