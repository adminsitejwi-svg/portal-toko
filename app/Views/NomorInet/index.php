                                    ...
                                    <?php if (!empty($MD_nomer_inet)) : ?>
                                        ...
                                        <?php foreach ($MD_nomer_inet as $row) : ?>
                                            <tr>
                                                ...
                                                <td><?= ($row['harga_layanan'] !== null && $row['harga_layanan'] !== '')
                                                        ? 'Rp ' . number_format((float) $row['harga_layanan'], 0, ',', '.') : '-' ?></td>
    ...
    '<?= esc($row['keterangan'], 'js') ?>'
                                ...
                                <div class="bg-white rounded-xl shadow-xl w-full max-w-2xl p-6 max-h-[90vh] overflow-y-auto">
                                    ...
                                    <form action="<?= site_url('NomorInet/update') ?>" method="POST">
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-5 gap-y-4">
                                            ...
                                            <!-- Kode Layanan Vendor (full) -->
                                            <div class="md:col-span-2">
                                                <label class="block text-sm font-medium text-gray-600 mb-1.5">Kode Layanan Vendor</label>
                                                <select id="edit_layanan_vendor_id" name="layanan_vendor_id"
                                            ...
                                            <div>
                                                ...
                                                <input type="text" id="edit_nama_vendor_display" disabled
                                                    class="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-lg bg-gray-100 outline-none">
                                            </div>
                                            <div>
                                                <label class="block text-sm font-medium text-gray-600 mb-1.5">Nama Layanan</label>
                                                <input type="text" id="edit_nama_layanan_display" disabled
                                                    class="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-lg bg-gray-100 outline-none">
                                            </div>
                                            ...
                                            <div>
                                                ...
                                                <input type="password" id="edit_password_inet" name="password_inet"
                                                    placeholder="Kosongkan jika tidak diubah" autocomplete="new-password"
                                                    class="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-lg focus:border-primary-500 focus:ring-2 focus:ring-primary-100 outline-none transition">
                                            </div>
                                            <div class="w-full md:col-span-2">
                                                ...
                                                <select id="edit_status" name="status"
                                                    class="block w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-lg bg-white focus:border-primary-500 focus:ring-2 focus:ring-primary-100 outline-none transition">
        ...
        <script>
            ...
            const editKodeSel = document.getElementById('edit_layanan_vendor_id');

            function fillEditDisplay() {
                const opt = editKodeSel.options[editKodeSel.selectedIndex];
                document.getElementById('edit_nama_vendor_display').value = opt ? (opt.getAttribute('data-vendor') || '') : '';
                document.getElementById('edit_nama_layanan_display').value = opt ? (opt.getAttribute('data-layanan') || '') : '';
            }
            editKodeSel.addEventListener('change', fillEditDisplay);

            function openEditModal(id, layanan_vendor_id, kecepatan_bandwidth, harga_layanan, nomor_inet, status, keterangan) {
                document.getElementById('edit_id').value = id;
                editKodeSel.value = layanan_vendor_id;
                fillEditDisplay();

                document.getElementById('edit_nomor_inet').value = nomor_inet;
                document.getElementById('edit_status').value = status;
                document.getElementById('edit_keterangan').value = keterangan;

                // bandwidth: tambahkan option kalau belum ada
